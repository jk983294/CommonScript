...
template <typename T, typename... Ts> struct MyTuple<T, Ts...> : MyTuple<Ts...> {
    MyTuple(T t, Ts... ts) : MyTuple<Ts...>(ts...), tail(t) {}
    T tail;
};
template <size_t, typename> struct elem_type_holder;
template <typename T, typename... Ts> struct elem_type_holder<0, MyTuple<T, Ts...> > {
    typedef T type;
};
template <size_t k, typename T, typename... Ts> struct elem_type_holder<k, MyTuple<T, Ts...> > {
...
typename std::enable_if<k != 0, typename elem_type_holder<k, MyTuple<T, Ts...> >::type&>::type get(MyTuple<T, Ts...>& t) {
    MyTuple<Ts...>& base = t;
    return get<k - 1>(base);
}
