...
docker build -t zerg/myapp .                                # -t name
docker build -t zerg/myapp:v1 .                             # -t name
...
# 部署私有仓库
docker run -d -p 5000:5000 --name my_repo registry
docker image tag zerg/myapp localhost:5000/zerg/myapp
docker push localhost:5000/zerg/myapp
docker pull localhost:5000/zerg/myapp

# test
docker run -it -v /opt/version/latest/:/opt/version/latest/ zerg/myapp
